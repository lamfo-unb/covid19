...
# pop brasil = 211.289.547
valores_iniciais <- calcular_valores_iniciais(
  infectados = 1
)

parametros <- calcular_parametros()
tempo <- 1:100
output <- lsoda(valores_iniciais, tempo, SEIR, parametros)
...
previsoes_ <- renomear_variaveis(previsoes)
...
prev_long <- pivotear_long(previsoes_)
...
previsoes_ <- processar_previsoes(
  previsoes = previsoes, col_time = 'time',
